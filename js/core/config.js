...
"use strict";
  ...
  // posiciones fijas confirmadas: almacén central (25,15), Nor-Oeste (12,38), Este (55,27)
  const WAREHOUSES = {
    central:  { id:'central',  name:'Almacén Central',    pos:{x:25,y:15}, infinite:true, dispatchedToday:0 },
    noroeste: { id:'noroeste', name:'Almacén Nor-Oeste',  pos:{x:12,y:38}, capacity:1000, stock:760, dispatchedToday:0 },
    este:     { id:'este',     name:'Almacén Este',       pos:{x:55,y:27}, capacity:1000, stock:840, dispatchedToday:0 },
  };

  const PRIORITY_BUCKETS = [4, 8, 12, 18, 36];

  const FALLA_TYPES = [
    { tipo:1, label:'tipo 1 (leve)',     minMin:20, maxMin:40  },
    { tipo:2, label:'tipo 2 (moderada)', minMin:45, maxMin:80  },
    { tipo:3, label:'tipo 3 (grave)',    minMin:90, maxMin:150 },
  ];
