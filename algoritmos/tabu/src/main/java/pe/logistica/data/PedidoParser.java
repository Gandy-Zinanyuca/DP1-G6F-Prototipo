...
import pe.logistica.model.Pedido;
...
public final class PedidoParser {
    public Pedido parsear(String linea, int anio, int mes) {
        ...
        if (partes.length != 2) throw new IllegalArgumentException("Pedido: se esperaba fecha:x,y,id,cantidad,plazo");
        String[] c = partes[1].split(",", -1);
        if (c.length != 5) throw new IllegalArgumentException("Pedido: se esperaban cinco campos");
        return new Pedido(c[2].trim(), ParserSupport.momento(partes[0], anio, mes),
                new Nodo(Integer.parseInt(c[0].trim()), Integer.parseInt(c[1].trim())),
                Integer.parseInt(c[3].trim()), Integer.parseInt(c[4].trim()));
    }
