...
public final class SolutionEvaluator {
    ...
    public EvaluacionSolucion evaluar(Solucion solucion, boolean exigirTodos) {
        ...
        var vistos = new HashSet<String>(); var vehiculosVistos = new HashSet<String>();
        ...
        if (exigirTodos) requeridos.keySet().stream().sorted().filter(id -> !vistos.contains(id))
                .forEach(id -> errores.add("Pedido no asignado: " + id));
    ...
    private ResultadoRuta evaluarRuta(Ruta ruta) {
        ...
        if (ruta.pedidos().isEmpty()) return new ResultadoRuta(ruta, t, t, List.of(), List.of(), 0, 0, 0, List.of());
        var errores = new ArrayList<String>();
        if (!v.disponible()) errores.add(v.codigo() + ": vehiculo no disponible");
        if (ruta.pedidos().stream().mapToLong(Pedido::cantidad).sum() > v.tipo().capacidad())
            errores.add(v.codigo() + ": capacidad excedida");
        ...
        for (Pedido p : ruta.pedidos()) if (p.fechaRegistro().isAfter(salida)) salida = p.fechaRegistro();
        ...
        for (Pedido p : ruta.pedidos()) {
            ...
            Visita visita = new Visita(p, camino.llegada(), inicio, inicio.plusMinutes(estado.parametros().servicioMinutos()),
