...
/**
 * Regresiones de continuidad operativa compartida, sin depender de la data
 * privada.
 */
public final class SimulacionComparadaTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var base = new EstadoOperacion(t, List.of(), List.of(new Vehiculo("TB01", n), new Vehiculo("TB02", n)),
                List.of(new Almacen("A", n, 16, false)), List.of(), List.of(), List.of(), List.of(), Set.of());
        var datos = new SimulacionComparada.Datos(base,
                List.of(new Pedido("p1", t, n, 8, 4), new Pedido("p2", t.plusHours(2), n, 8, 4)));
        ...
        try {
            ...
            for (var motor : ExperimentacionTest.motores()) {
                ...
                for (var linea : Files.readAllLines(csv))
                    ok(linea.split(",", -1).length == 19, "CSV uniforme incluso sin demanda");
                var repetido = SimulacionComparada.ejecutar(datos, motor, 10, 0, dir.resolve("repite-" + i + ".csv"), 7,
                        1);
                ok(repetido.completos() == r.completos() && repetido.instante().equals(r.instante()),
                        "Restaurar condiciones entre corridas");
                var limite = SimulacionComparada.ejecutar(datos, motor, 10, 1, dir.resolve("limite-" + i + ".csv"), 7,
                        1);
                ok(limite.fin().equals("LIMITE_DE_CICLOS") && limite.completos() == 0,
                        "Despacho no equivale a entrega");
                var colapso = SimulacionComparada.ejecutar(datos, motor, 10, 0, dir.resolve("colapso-" + i + ".csv"), 7,
                        2);
                ok(colapso.fin().equals("COLAPSO_PLANIFICACION") && colapso.completos() == 0,
                        "No despachar plan incompleto");
                ...
                var esperaTurno = SimulacionComparada.ejecutar(
                        new SimulacionComparada.Datos(baseMadrugada, List.of(pedidoCinco)), motor, 10, 31,
                        dir.resolve("turno-" + i + ".csv"), 7, 1);
