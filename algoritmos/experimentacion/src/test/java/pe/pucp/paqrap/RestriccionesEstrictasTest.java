...
public final class RestriccionesEstrictasTest {
    ...
    static ALNSPlanner alns() {
        return new ALNSPlanner(new ConfiguracionALNS(6, 6, 3, 2, .7, .05, 0, 7));
    }
    ...
    static void testTabu() {
        ...
        l.registrar(ida, 5, 3);
        ok(l.esTabu(TabuMove.asignacion("p", "b", "a"), 6), "inversa no tabu");
        ...
        ok(l.esTabu(TabuMove.relocate("v", "a", 2, 0), 2), "relocate inverso");
        pruebas++;
    }
    ...
    static void testDivision() {
        ...
        ok(resultado.solucion().rutas().stream().flatMap(r -> r.partes().stream()).mapToInt(PartePedido::cantidad)
                .sum() == 30, "cantidad perdida");
        pruebas++;
    }
    ...
    static void testPlazo() {
        ...
        ok(r.evaluacion().factible() && !r.evaluacion().completa() && r.metricas().paquetesPendientes() == 2,
                "plazo blando");
        var p = new Pedido("fin", T.minusHours(3).minusMinutes(30), N, 1, 4);
        var ev = new EvaluadorFactibilidad(simple(List.of(p)), parametros(60));
        ok(!ev.evaluar(una(ev)).factible(), "fin servicio fuera de plazo admitido");
        pruebas++;
    }
    ...
    static void testTurno() {
        ...
        var e = estado(List.of(p), List.of(v), List.of(new Almacen("A", N, 5, false)), List.of(), List.of(), List.of(),
                List.of(), Set.of("TA01"));
        ...
        ok(turno15.factible() && turno15.salida().equals(T.toLocalDate().atTime(15, 0)),
        ...
        ok(turno7.factible() && turno7.salida().equals(noche.toLocalDate().atTime(7, 0)),
    ...
    static void testDescanso() {
        ...
        for (var p : r.paradas())
            ok(!GestorDisponibilidad.solapa(p.llegada(), p.finServicio(), r.descansoInicio(), r.descansoFin()),
                    "servicio durante descanso");
        pruebas++;
    }
    ...
    static void testMantenimiento() {
        ...
        var ev = new EvaluadorFactibilidad(e, parametros(60));
        ok(!ev.evaluar(una(ev)).factible(), "mantenimiento durante ruta admitido");
        ok(!GestorDisponibilidad.solapa(T, T.plusHours(1), T.plusHours(1), T.plusHours(2)), "limite semiabierto");
        pruebas++;
    }
    ...
    static void testAveria() {
        ...
        var ev = new EvaluadorFactibilidad(e, parametros(60));
        ok(!ev.evaluar(una(ev)).factible(), "averia solapada");
        pruebas++;
    }
    ...
    static void testStock() {
        ...
        ok(!ev.evaluar(s).factible(), "stock agregado");
        pruebas++;
    }
    ...
    static void testBloqueos() {
        ...
        for (var paso : c.pasos())
            ok(mapa.cruceValido(paso.origen(), paso.destino(), paso.salida(), paso.llegada()), "arco bloqueado");
        ...
        ok(espera.pasos().get(0).salida().equals(T.plusMinutes(10)), "espera reapertura");
        pruebas++;
    }
    ...
    static void testIntegridad() {
        ...
        ok(original.almacenes().get(0).stock() == 100, "snapshot mutado");
        pruebas++;
    }
    ...
    static void testReplanificacion() {
        ...
        ok(r.evaluacion().completa() && r.solucion().rutas().contains(activa), "no conserva carga despachada");
        ...
        ok(auxilio.evaluacion().completa() && auxilio.solucion().rutas().stream()
                .anyMatch(x -> x.vehiculo().equals("TA02") && !x.partes().isEmpty()), "no reasigna averia");
        pruebas++;
    }

    static void testComparabilidad() {
        ...
        ok(ts0.metricas().iteracionMejor() == 0 && al0.metricas().iteracionMejor() == 0,
                "inicial debe registrarse en iteracion cero");
        var ts = tabu().planificar(e, parametros(10));
        var al = alns().planificar(e, parametros(10));
        for (var resultado : List.of(ts, al)) {
            ...
            var prefijo = resultado.algoritmo().startsWith("TS")
                    ...
                    : new ALNSPlanner(new ConfiguracionALNS(mejor, 6, 3, 2, .7, .05, 0, 7)).planificar(e,
                            parametros(10));
            if (resultado.algoritmo().startsWith("TS"))
                ok(prefijo.solucion().equals(resultado.solucion()),
                        "TS no recupera la mejor solucion en la iteracion registrada");
        }
        ...
        ok(ts.evaluacion().objetivo() <= ts0.evaluacion().objetivo()
                && al.evaluacion().objetivo() <= al0.evaluacion().objetivo(), "empeora inicial");
        ok(ts.metricas().candidatosEvaluados() > 0 && al.metricas().candidatosEvaluados() > 0, "sin candidatos");
        pruebas++;
    }
    ...
    static void testValidacionEntrada() {
        boolean fallo = false;
        try {
            simple(List.of(new Pedido("f", T.plusMinutes(1), N, 1, 4)));
        } catch (IllegalArgumentException ex) {
            fallo = true;
        }
        ok(fallo, "pedido futuro admitido");
        fallo = false;
        try {
            new Pedido("x", T, N, 1, 7);
        } catch (IllegalArgumentException ex) {
            fallo = true;
        }
        ok(fallo, "plazo no permitido");
        pruebas++;
    }
}
