...
public final class AlimentacionTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int hora : new int[]{13,21,5}) {
            ...
            verificar(rr.factible() && rr.paradas().get(0).finServicio().equals(t.plusHours(1)), "Entrega antes de alimentacion tardia");
            verificar(rr.descansoInicio().equals(t.plusHours(1)) && rr.descansoFin().equals(t.plusHours(2)), "Alimentacion hasta cambio de turno");
        }
        ...
        verificar(SimulacionComparada.descansosCompletados(base,base.vehiculos(),dia.atTime(8,59),par,finales).isEmpty(), "No acreditar 59 minutos");
        verificar(SimulacionComparada.descansosCompletados(base,base.vehiculos(),dia.atTime(9,0),par,finales).contains("TA01"), "Acreditar hora ociosa completa");
        verificar(SimulacionComparada.descansosCompletados(base,base.vehiculos(),dia.atTime(15,0),par,finales).isEmpty(), "No heredar descanso de otro turno");
        finales.clear();
        var ocupada = List.of(new Vehiculo("TA01",TipoVehiculo.TA,n,true,dia.atTime(12,0)));
        verificar(SimulacionComparada.descansosCompletados(base,ocupada,dia.atTime(12,30),par,finales).isEmpty(), "No contar tiempo de ruta como ocio");
        verificar(SimulacionComparada.descansosCompletados(base,ocupada,dia.atTime(13,0),par,finales).contains("TA01"), "Descanso despues del retorno");
        finales.clear(); finales.put("TA01",dia.atTime(13,0));
        verificar(SimulacionComparada.descansosCompletados(base,ocupada,dia.atTime(12,30),par,finales).isEmpty(), "No acreditar descanso futuro comprometido");
        // Pedido real de la linea 352, aislado con una unidad que estuvo libre por la mañana.
        ...
        try {
            ...
            for (var motor : ExperimentacionTest.motores()) {
                ...
                verificar(resultado.fin().equals("FIN_DE_DATOS") && resultado.completos()==1 && resultado.paquetesEntregados()==7,
                        "Pedido de las 12:02 debe completarse con alimentacion previa");
            }
        } finally {
            try (var archivos = java.nio.file.Files.walk(archivo)) {
                for (var ruta : archivos.sorted(Comparator.reverseOrder()).toList()) java.nio.file.Files.delete(ruta);
            }
        }
