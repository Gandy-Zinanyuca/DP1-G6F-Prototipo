...
/** Regresiones de medicion, completitud y exportacion pareada con archivos temporales. */
public final class ExperimentacionTest {
 static void ok(boolean b,String texto) {if(!b) throw new AssertionError(texto);}
 ...
 public static void main(String[] args) throws Exception {
  ...
  var a=new PartePedido("a",p,4); var b=new PartePedido("b",p,4);
  ...
  var r2=new ResultadoRuta(new Ruta("TA02","A",List.of(b),false),T,T.plusMinutes(100),"A",
   List.of(new Parada(List.of(b),T.plusMinutes(40),T.plusMinutes(90))),List.of(),null,null,0,0,List.of());
  ...
  ok(Holguras.calcular(List.of(p),List.of(r1)).promedioMin()==null,"No promediar pedido parcialmente entregado");
  var e=new EstadoOperacion(T,List.of(p),List.of(new Vehiculo("TA01",N)),List.of(new Almacen("A",N,100,false)),
  ...
  for(var motor:motores()) {
   ...
   ok(completo.metricas().taPrimeraCompletaMs()!=null && completo.metricas().taPrimeraCompletaMs()<=completo.metricas().taMs(),"Tiempo primera completa");
   var sinStock=new EstadoOperacion(T,e.pedidos(),e.vehiculos(),List.of(new Almacen("A",N,0,false)),List.of(),List.of(),List.of(),List.of(),Set.of());
   var colapso=motor.planificar(sinStock,ParametrosOperacion.porDefecto());
   ok(colapso.metricas().colapso() && colapso.metricas().holguraPromedioMin()==null && colapso.metricas().taPrimeraCompletaMs()==null,"Colapso sin ceros artificiales");
   var vacio=new EstadoOperacion(T,List.of(),e.vehiculos(),e.almacenes(),List.of(),List.of(),List.of(),List.of(),Set.of());
   var sinDemanda=motor.planificar(vacio,ParametrosOperacion.porDefecto());
   ok(sinDemanda.metricas().estadoResultado().equals("SIN_DEMANDA") && sinDemanda.metricas().holguraPromedioMin()==null,"Sin demanda no es colapso");
  }
  ...
  var costoso=new ParametrosOperacion(parametros.servicioMinutos(),true,parametros.turnoMinutos(),parametros.inicioTurnoMinuto(),
   parametros.descansoDesde(),parametros.descansoHasta(),parametros.descansoMinutos(),parametros.tamanioParte(),
   1000000000,1,parametros.velocidades());
  ok(rapido.evaluar(sol).objetivo()==new EvaluadorFactibilidad(e,costoso).evaluar(sol).objetivo(),"El costo no debe cambiar el objetivo");
  var parcial=new Solucion(List.of(new Ruta("TA01","A",List.of(partes.get(0)),false)),List.of(partes.get(1)));
  ok(rapido.evaluar(sol).objetivo()<rapido.evaluar(parcial).objetivo(),"Completitud tiene prioridad");
  var tarde=new EstadoOperacion(T,e.pedidos(),List.of(new Vehiculo("TA01",TipoVehiculo.TA,N,true,T.plusMinutes(20))),e.almacenes(),List.of(),List.of(),List.of(),List.of(),Set.of());
  ok(rapido.evaluar(sol).objetivo()<new EvaluadorFactibilidad(tarde,ParametrosOperacion.porDefecto()).evaluar(sol).objetivo(),"Objetivo favorece holgura");
  var dir=Files.createTempDirectory("paqrap-experimento-");
  try {
   var ventas=dir.resolve("ventas.202609.txt"); var bloqueos=dir.resolve("bloqueo.2609.txt"); var mant=dir.resolve("mant.txt");
   Files.writeString(ventas,"01d08h00m:25,15,c1,04,4\n"); Files.writeString(bloqueos,""); Files.writeString(mant,"");
   String[] base={ventas.toString(),bloqueos.toString(),mant.toString(),T.toString()};
   var comando=new ArrayList<String>(List.of(base));
   comando.addAll(List.of(dir.resolve("pares").toString(),"2","7,8","0","--escenario","E2","--factor-carga","2","--instancia","caso,con-coma"));
   ...
   for(int i=1;i<filas.size();i+=2) {
    var ts=filas.get(i).split(separador,-1); var al=filas.get(i+1).split(separador,-1);
    ok(ts.length==39 && al.length==39,"Ancho CSV igual a cabecera");
    for(int campo:List.of(0,1,2,3,4,5,6,8)) ok(ts[campo].equals(al[campo]),"Par no corresponde a misma instancia y semilla");
   }
   ...
   var imposible=new EstadoOperacion(T,entrada.estado.pedidos(),List.of(),entrada.estado.almacenes(),List.of(),List.of(),List.of(),List.of(),Set.of());
   ...
   ok(campos[15].isEmpty() && campos[16].isEmpty() && campos[17].isEmpty(),"Holguras y primera completa deben quedar vacias en colapso");
   for(var motor:motores()) {
    ...
    ok(uno.solucion().equals(dos.solucion()) && uno.metricas().holguraPromedioMin().equals(dos.metricas().holguraPromedioMin()),"Reproducibilidad");
   }
  } finally {
   // Solo borra el directorio temporal exclusivo creado por esta prueba.
   try(var archivos=Files.walk(dir)) {for(var archivo:archivos.sorted(Comparator.reverseOrder()).toList()) Files.delete(archivo);}
  }
