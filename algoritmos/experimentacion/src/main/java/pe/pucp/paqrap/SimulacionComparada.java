...
/** Reloj externo a los motores: ingresos, despachos, entregas y retornos persistentes.
 * Cada algoritmo/semilla recibe una simulacion nueva de la misma demanda.
 * Un plan incompleto detiene la corrida; no prueba inviabilidad matematica.
 */
public final class SimulacionComparada {
    public record Datos(EstadoOperacion base, List<Pedido> pedidos) {
        public Datos { pedidos = List.copyOf(pedidos); }
    }
    public record Resumen(String fin, LocalDateTime instante, int ciclos, int completos,
                          int paquetesEntregados, double holguraMedia, double holguraMinima,
                          int ejecuciones, double taTotalMs, double taMaxMs, double distanciaKm,
    ...
    private static LocalDateTime turno(LocalDateTime t, ParametrosOperacion p) {
        ...
        return t.toLocalDate().atStartOfDay().plusMinutes(p.inicioTurnoMinuto()
                + Math.floorDiv(m - p.inicioTurnoMinuto(), p.turnoMinutos()) * p.turnoMinutos());
    }

    /** Mantiene cantidades por pedido original; una entrega solo completa al recibir la ultima parte. */
    public static Resumen ejecutar(Datos datos, PlanificadorEstricto motor, int sa, int maxCiclos,
            Path archivo, long semilla, double factor) throws IOException {
        ...
        for (var p : demanda) {
            ...
            if (cantidad > Integer.MAX_VALUE) throw new IllegalArgumentException("Carga excesiva");
            var q = new Pedido(p.id(), p.fechaRegistro(), p.ubicacion(), (int)cantidad, p.plazoHoras(), p.clienteId());
            if (originales.put(q.id(), q) != null) throw new IllegalArgumentException("Pedido duplicado");
        }
            ...
            while (true) {
                for (var it = eventos.iterator(); it.hasNext();) {
                    ...
                    if (total == original.cantidad()) {
                        ...
                        sumaHolgura += holgura; minima = Math.min(minima, holgura);
                    }
                    it.remove();
                }
                ...
                for (var v : flota) if (v.disponibleDesde().isAfter(t)) retornaron = false;
                ...
                if (resultado != null && !colapso) for (var ruta : resultado.evaluacion().rutas()) {
                    if (ruta.ruta().partes().isEmpty() || !ruta.salida().isBefore(siguiente)) continue;
                    despachadas++;
                    distancia += ruta.distanciaKm(); minutos += ruta.minutos();
                    ...
                    for (var parada : ruta.paradas()) for (var parte : parada.partes()) {
                        ...
                        if (restante == 0) pendientes.remove(p.id());
                        else pendientes.put(p.id(), new Pedido(p.id(), p.fechaRegistro(), p.ubicacion(), restante, p.plazoHoras(), p.clienteId()));
                    }
                    ...
                    var destino = almacenes.stream().filter(a -> a.id().equals(ruta.almacenRetorno())).findFirst().orElseThrow();
                ...
                csv.write(semilla + "," + factor + "," + ciclos + "," + t + "," + (colapso ? "COLAPSO_PLANIFICACION" : m == null ? "SIN_DEMANDA" : "COMPLETA")
                        + "," + (m == null ? "0,,,,,,,0,0" : m.taMs() + "," + celda(m.holguraPromedioMin()) + "," + celda(m.holguraMinimaMin()) + "," + m.distanciaKm() + "," + m.tiempoRutasMinutos() + "," + m.vehiculosUsados() + "," + m.utilizacionCapacidad() + "," + m.pedidosTotales() + "," + m.paquetesPendientes())
                        + "," + despachadas + "," + completos + "," + paquetes + "," + (completos == 0 ? "," : sumaHolgura / completos + "," + minima) + "\n");
                ciclos++;
                if (colapso) { fin = "COLAPSO_PLANIFICACION"; break; }
                if (!siguiente.toLocalDate().equals(t.toLocalDate())) almacenes = new ArrayList<>(base.almacenes());
                t = siguiente;
            }
        }
        return new Resumen(fin, t, ciclos, completos, paquetes,
                completos == 0 ? Double.NaN : sumaHolgura / completos, completos == 0 ? Double.NaN : minima,
                ejecuciones, taTotal, taMax, distancia, minutos, utilizados.size(),
                capacidadDespachada == 0 ? 0 : (double)cargaDespachada / capacidadDespachada);
    }

    private static String celda(Double valor) { return valor == null ? "" : valor.toString(); }
    private static String minutos(double valor) { return Double.isNaN(valor) ? "N/D" : String.format(Locale.ROOT, "%.2f", valor); }

    /** Carga meses consecutivos; exige bloqueos de cada mes para no omitir incidencias silenciosamente. */
    private static Datos cargar(Path ventas, Path bloqueos, Path mantenimiento, YearMonth mes) throws IOException {
        ...
        for (var actual = mes; ; actual = actual.plusMonths(1)) {
            ...
            if (!Files.exists(v)) break;
            Path b = bloqueos.resolve(String.format(Locale.ROOT, "bloqueo.%02d%02d.txt", actual.getYear() % 100, actual.getMonthValue()));
            if (base == null) base = new DatasetLoader().cargar(v, b, mantenimiento, inicio, 48, Integer.MAX_VALUE).estado();
        ...
        if (base == null) throw new IllegalArgumentException("No hay ventas del mes inicial");
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 11) {
            ...
            var b = datos.base();
            datos = new Datos(new EstadoOperacion(b.instante(), b.pedidos(), b.vehiculos(), b.almacenes(), b.bloqueos(), averias, b.mantenimientos(), b.rutasEnCurso(), b.descansoRealizado()), datos.pedidos());
        }
        ...
        try (var resumen = Files.newBufferedWriter(salida.resolve("resumen.csv"), StandardCharsets.UTF_8)) {
            ...
            for (String s : semillas.split(",")) {
                ...
                if (!usadas.add(semilla)) throw new IllegalArgumentException("Semilla repetida");
                for (String algoritmo : seleccion.equals("AMBOS") ? List.of("TS", "ALNS") : List.of(seleccion)) {
                    ...
                    var r = ejecutar(datos, motor, sa, ciclos, salida.resolve(algoritmo + "-" + semilla + ".csv"), semilla, factor);
                    double dias = Duration.between(datos.base().instante(), r.instante()).toMinutes() / 1440.0;
                    resumen.write(algoritmo + "," + semilla + "," + factor + "," + r.fin() + "," + r.instante() + "," + dias + "," + r.ciclos() + "," + r.completos() + "," + r.paquetesEntregados() + "," + (Double.isNaN(r.holguraMedia()) ? "," : r.holguraMedia() + "," + r.holguraMinima())
                            + "," + r.ejecuciones() + "," + r.taTotalMs() + "," + (r.ejecuciones() == 0 ? "" : r.taTotalMs() / r.ejecuciones()) + "," + r.taMaxMs() + "," + r.distanciaKm() + "," + r.minutosRutas() + "," + r.vehiculos() + "," + r.utilizacion() + "\n");
                    resumen.flush();
                    System.out.printf(Locale.ROOT, "%s | %s | %.3f dias | %d pedidos entregados | %d paquetes | holgura real media/min: %s / %s min%n", algoritmo, r.fin(), dias, r.completos(), r.paquetesEntregados(), minutos(r.holguraMedia()), minutos(r.holguraMinima()));
                    System.out.printf(Locale.ROOT, "Ta total/media/max: %.2f / %.2f / %.2f ms | %.2f km | %.2f min rutas | %d vehiculos | capacidad %.1f%%%n",
                            r.taTotalMs(), r.ejecuciones() == 0 ? 0 : r.taTotalMs() / r.ejecuciones(), r.taMaxMs(), r.distanciaKm(), r.minutosRutas(), r.vehiculos(), 100 * r.utilizacion());
                }
            }
        }
