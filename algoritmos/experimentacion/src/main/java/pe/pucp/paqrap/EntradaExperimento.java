...
/**
 * Preparacion externa a los motores; opciones despues de los argumentos
 * posicionales.
 */
final class EntradaExperimento {
    ...
    EntradaExperimento(String[] entrada) throws Exception {
        int corte = 0;
        while (corte < entrada.length && !entrada[corte].startsWith("--"))
            corte++;
        args = Arrays.copyOf(entrada, corte);
        if (args.length < 4)
            throw new IllegalArgumentException("Se requieren ventas, bloqueos, - e instante ISO");
        var permitidas = Set.of("escenario", "carga", "instancia", "factor-carga", "max-pedidos", "horizonte-horas",
                "salida");
        for (int i = corte; i < entrada.length; i += 2) {
            if (i + 1 >= entrada.length || !entrada[i].startsWith("--"))
                throw new IllegalArgumentException("Opcion sin valor");
        ...
        if (!Set.of("E1", "E2", "E3").contains(escenario))
            throw new IllegalArgumentException("Escenario E1, E2 o E3");
        factor = Double.parseDouble(opciones.getOrDefault("factor-carga", "1"));
        if (!Double.isFinite(factor) || factor <= 0)
            throw new IllegalArgumentException("Factor positivo");
        ...
        carga = new DatasetLoader().cargarExperimental(Path.of(args[0]), Path.of(args[1]), LocalDateTime.parse(args[3]),
                horizonte, maxPedidos);
        var e = carga.estado();
        ...
        for (var p : e.pedidos()) {
            ...
            pedidos.add(new Pedido(p.id(), p.fechaRegistro(), p.ubicacion(), (int) cantidad, p.plazoHoras(),
                    p.clienteId()));
        }
        estado = new EstadoOperacion(e.instante(), pedidos, e.vehiculos(), e.almacenes(), e.bloqueos(), List.of(),
                List.of(), e.rutasEnCurso(), e.descansoRealizado());
    }

    int iteraciones(int pos, int defecto) {
        int n = args.length > pos ? Integer.parseInt(args[pos]) : defecto;
        if (n < 0)
            throw new IllegalArgumentException("Iteraciones negativas");
        return n;
    }
