...
public final class CompararAlgoritmos {
 public static void main(String[] args) throws Exception {
  if(args.length<5 || args.length>7)throw new IllegalArgumentException(
   "Uso: CompararAlgoritmos ventas.AAAAMM.txt bloqueo.AAMM.txt mantenimiento.txt instante-ISO directorio-salida [iteraciones] [semillas-separadas-por-coma]");
  ...
  metadata.append("\nCarga: ").append(carga.pedidosLeidos()).append(" pedidos; futuros excluidos: ").append(carga.futurosExcluidos())
   ...
   .append(Math.max(1,iteraciones)).append(". Ambos sin parada por reloj. Semillas: ").append(semillas).append(".\n\n")
  ...
  for(String semilla:semillas.split(",")) {
   ...
   List<PlanificadorEstricto> motores=List.of(
    new TabuSearchPlanner(new ConfiguracionTabu(iteraciones,7,Math.max(1,iteraciones),400,0,seed)),
    new ALNSPlanner(new ConfiguracionALNS(iteraciones,Math.max(1,iteraciones),4,5,.7,.05,0,seed)));
