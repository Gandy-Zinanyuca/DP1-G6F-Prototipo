...
/** Una combinacion escenario/carga/instancia; cada semilla produce un par TS-ALNS. */
public final class CompararAlgoritmos {
 public static void main(String[] args) throws Exception {
  ...
  if(e.args.length<5 || e.args.length>8 || e.opciones.containsKey("salida"))
   throw new IllegalArgumentException("Uso: CompararAlgoritmos ventas bloqueos mantenimiento instante salida [iteraciones] [semillas-con-comas] [presupuesto-ms] [--opcion valor]");
  ...
  for(String semilla:semillas.split(",",-1)) seeds.add(Long.parseLong(semilla.trim()));
  if(new HashSet<>(seeds).size()!=seeds.size()) throw new IllegalArgumentException("Semillas repetidas; usar semillas distintas para las repeticiones");
  ...
  for(long seed:seeds) {
   ...
   for(var motor:motores) {
    ...
    if(!e.estado.toString().equals(estadoAntes)) throw new AssertionError("Estado original alterado");
    ReporteExperimento.auditar(e,r);
    if(r.evaluacion().objetivo()>objetivoInicial+1e-9) throw new AssertionError("Empeora inicial comun");
    var fila=ReporteExperimento.fila(e,r,seed,repeticion,iter,presupuesto);
    for(String nombre:List.of("metricas",r.algoritmo()))
     Files.writeString(salida.resolve(nombre+".csv"),fila,StandardCharsets.UTF_8,StandardOpenOption.APPEND);
  ...
  for(var nombre:colapsos.keySet()) System.out.printf(Locale.ROOT,
   "%s: completas %d/%d | colapsos %d/%d | sin demanda %d%n",nombre,completas.get(nombre),seeds.size(),
   colapsos.get(nombre),seeds.size(),seeds.size()-completas.get(nombre)-colapsos.get(nombre));
 }
}
