...
public final class Resultados {
  public static ResultadoPlanificacion crear(String nombre, Solucion s, EvaluadorFactibilidad ev, long inicio,
      int iter, int iteracionMejor, long candidatos, long iniciales, String parada,
    ...
    var m = new MetricasResultado((System.nanoTime() - inicio) / 1e6, evaluacion.objetivo(), evaluacion.costo(),
        ...
        total == 0 ? 1 : (double) completos / total,
        paquetes == 0 ? 1 : (double) carga / paquetes, usados, capacidad == 0 ? 0 : (double) carga / capacidad, iter, iteracionMejor,
        candidatos, iniciales,
        total, completos, evaluacion.paquetesPendientes(), parada, estadoResultado,
        evaluacion.completa() ? holguras.promedioMin() : null,
        evaluacion.completa() ? holguras.minimaMin() : null,
        taPrimeraCompletaMs, iteracionPrimeraCompleta);
