...
/** Una observacion por pedido completo: deadline menos fin de servicio de su ultima parte. */
public final class Holguras {
 ...
 public static Resumen calcular(List<Pedido> pedidos, List<ResultadoRuta> rutas) {
  ...
  for(var ruta:rutas) if(ruta.factible()) for(var parada:ruta.paradas()) for(var parte:parada.partes()) {
  ...
  int n=0; double suma=0,minima=Double.POSITIVE_INFINITY;
  for(var p:pedidos) if(cantidades.getOrDefault(p.id(),0)==p.cantidad()) {
   double margen=Duration.between(finales.get(p.id()),p.deadline()).toMillis()/60000.0;
   suma+=margen; minima=Math.min(minima,margen); n++;
  }
