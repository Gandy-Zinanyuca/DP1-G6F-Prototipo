...
public record EstadoOperacion(LocalDateTime instante, List<Pedido> pedidos, List<Vehiculo> vehiculos,
  ...
  public EstadoOperacion {
    ...
    for(var a:averias) 
      if(!vs.contains(a.vehiculo())) throw new IllegalArgumentException("Averia de unidad desconocida");
    for(var m:mantenimientos) 
      if(!vs.contains(m.vehiculo())) throw new IllegalArgumentException("Mantenimiento de unidad desconocida");
    ...
    for(var p:pedidos) 
      if(p.fechaRegistro().isAfter(instante)) throw new IllegalArgumentException("El backend debe excluir pedidos futuros");
  }
  private static void unicos(List<String> ids) { 
    if(new HashSet<>(ids).size()!=ids.size()) throw new IllegalArgumentException("Identificador duplicado"); 
  }
}
